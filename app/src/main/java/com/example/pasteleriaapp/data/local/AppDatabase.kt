            ...
            override fun onCreate(db: SupportSQLiteDatabase) {
                ...
                INSTANCE?.let { database ->
                    CoroutineScope(SupervisorJob() + Dispatchers.IO).launch {
                        populateDatabase(database.categoriaDao(), database.productoDao())
                    }
                }
            }

            suspend fun populateDatabase(categoriaDao: CategoriaDao, productoDao: ProductoDao) {
                productoDao.eliminarTodosLosProductos()
                categoriaDao.eliminarTodasLasCategorias()

                // --- INSERTAR CATEGORÍAS (con ID 0) ---
                val categorias = listOf(
                    // Room reemplazará el '0' por '1'
                    CategoriaEntity(0, "Tortas Cuadradas", "torta_cuadrada_de_chocolate"),
                    // Room reemplazará el '0' por '2'
